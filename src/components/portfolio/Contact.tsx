...
const testimonials = [
  {
    quote: "Feri shipped our entire marketing site in two weeks. The level of polish is unreal.",
    name: "Arya Dinatha",
    role: "Founder, Northwind",
  },
  {
    quote: "Rare combination — strong eye for design and rigorous, well-documented code.",
    name: "David Hartono",
    role: "Engineering Lead, Stack OS",
  },
  {
    quote: "Reliable, fast, and proactive. He treats every project like it's his own product.",
    name: "Ayu Pratiwi",
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_1.1fr] lg:gap-16">
          <ScrollReveal variant="stagger-container" className="flex flex-col">
            ...
            <ScrollReveal variant="stagger-item">
              <p className="mt-5 max-w-md text-base leading-relaxed text-muted-foreground">
                I'm currently taking on a few new projects. The fastest way to reach me is by email
                — I'll get back within a day.
              </p>
            </ScrollReveal>

            <ScrollReveal variant="stagger-item" className="flex">
              <a
                href="mailto:pandedwiwjy@gmail.com subject=Personal Website"
                className="group mt-8 inline-flex items-center gap-3 rounded-2xl border border-hairline bg-card px-5 py-4 transition-colors hover:border-brand/40 w-full sm:w-auto"
              >
            ...
            <ScrollReveal variant="stagger-container" className="mt-8 grid grid-cols-2 gap-3">
              {socials.map((s) => {
                const Icon = s.icon;
                return (
                  <ScrollReveal key={s.name} variant="stagger-item">
                    <a
                      href={s.href}
          ...
          <ScrollReveal variant="stagger-container" className="flex flex-col">
            ...
            <div className="mt-3 flex flex-col gap-4">
              {testimonials.map((t) => (
                <ScrollReveal key={t.name} variant="stagger-item">
                  <figure className="rounded-2xl border border-hairline bg-card p-6 transition-colors hover:border-brand/40">
                    <div className="flex gap-0.5 text-brand">
                      {Array.from({ length: 5 }).map((_, i) => (
                        <Star key={i} className="h-3.5 w-3.5 fill-current" />
                      ))}
                    </div>
                    <blockquote className="mt-4 text-left text-base leading-relaxed text-foreground">
                      "{t.quote}"
                    </blockquote>
